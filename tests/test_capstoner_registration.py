...
from tests.test_author_account_links import author_db, author_postgres, ADVISER, AUTHOR

ROOT = Path(__file__).resolve().parents[1]
pages = import_module("app.routes.pages.profile")
...
def test_browser_registration_review_and_author_assignment(workflow_browser):
    ...
    sign_in(client, 4)
    page.goto(f"{client.browser_url}/capstoners")
    for field, value, label in [("user_id", "1", "User account"), ("credit", f"1:{credit}", "Unlinked author credit")]:
        option_text = page.locator(f'#{field} option[value="{value}"]').text_content()
        combo = page.get_by_role("combobox", name=label, exact=True)
        combo.click()
        combo.fill(option_text)
