...
The current settings put these folders under `instance/uploads`. These files are outside Flask's public static directory and are served only through protected routes. Admins download COR PDFs through the protected verification route. Validation is not a malware scan. Files are retained after review or request deletion; account/file retention cleanup is not automatic. Older accounts without a COR remain reviewable.
