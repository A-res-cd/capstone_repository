...
def parse_cor_text(text):
    ...
    academic_year = re.search(r"\b(20\d{2})\s*-\s*(20\d{2})\b", text)
    fields["academic_year"] = f"{academic_year.group(1)}-{academic_year.group(2)}" if academic_year else ""

    term = re.search(r"\b(1st|2nd|3rd|4th)\s+Semester\b", text, re.IGNORECASE)
    fields["term"] = term.group(1).lower() + " Semester" if term else ""

    year_level = re.search(
        r"Year\s*Level\s*:\s*(\d+)\s*(?:st|nd|rd|th)?\s*Year?\b",
        text,
        re.IGNORECASE,
    )
    if not year_level:
        ...
        word_match = re.search(
            r"Year\s*Level\s*:\s*(first|second|third|fourth|fifth)\s*Year?\b",
            text,
            re.IGNORECASE,
        )
        fields["year_level"] = str(year_words[word_match.group(1).lower()]) if word_match else ""
    else:
        fields["year_level"] = year_level.group(1)
